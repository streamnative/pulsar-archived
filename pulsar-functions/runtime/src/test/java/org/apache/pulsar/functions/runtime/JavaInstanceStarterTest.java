...
public class JavaInstanceStarterTest {
    ...
    @Test
    public void jCommanderShouldSetValueToNullIfConfigNotProvided() throws IOException {
        JavaInstanceStarter javaInstanceStarter = new JavaInstanceStarter();
        String[] args = new CommandLineArgsBuilder()
                .withJCommanderConfigs("--transform_function_id", STRING_INVALID_VALUE)
                .build();
        JCommander jcommander = new JCommander(javaInstanceStarter);
        // parse args by JCommander
        jcommander.parse(args);

        Assert.assertEquals(javaInstanceStarter.transformFunctionId, STRING_INVALID_VALUE);
        // verify if only use JCommander, the setting will set the String and Integer and Boolean type to null
        // Required
        Assert.assertNull(javaInstanceStarter.expectedHealthCheckInterval);
        // Optional
        Assert.assertNull(javaInstanceStarter.clientAuthenticationPlugin);
        Assert.assertNull(javaInstanceStarter.useTls);
        Assert.assertFalse(javaInstanceStarter.exposePulsarAdminClientEnabled);
    }

    @Test
    public void fileShouldSetValueToNullIfConfigNotProvided() throws IOException {
        JavaInstanceStarter javaInstanceStarter = new JavaInstanceStarter();
        String[] args = new CommandLineArgsBuilder()
        ...
        // parse args by JCommander
        jcommander.parse(args);
        javaInstanceStarter.completeConfigFromFileIfCmdArgNotProvided();
        // verify if only use JCommander, the setting will set the String and Integer and Boolean type to null
        // Optional
        Assert.assertNull(javaInstanceStarter.clientAuthenticationPlugin);
        Assert.assertNull(javaInstanceStarter.useTls);
    ...
    @Test
    public void missingRequiredFieldShouldThrowIllegalArgumentException() throws IOException {
        JavaInstanceStarter javaInstanceStarter = new JavaInstanceStarter();
        String[] args = new CommandLineArgsBuilder()
                .withJCommanderConfigs("--transform_function_id", STRING_INVALID_VALUE)
                .build();
        Assert.expectThrows(IllegalArgumentException.class, () -> javaInstanceStarter.setConfigs(args));
    }

    @Test
    public void provideRequiredFieldsOnlyShouldSetValueProperlyAndOptionalFieldsAreSetToDefault() throws IOException {
        JavaInstanceStarter javaInstanceStarter = new JavaInstanceStarter();
        String[] args = new CommandLineArgsBuilder()
                .withJCommanderConfigs(requiredJCommanderConfigs())
                .build();
    ...
    public void useBooleanStringShouldSetArityOneBooleanField() throws IOException {
        ...
        javaInstanceStarter.setConfigs(args);
        assertRequiredFieldsAreSet(javaInstanceStarter);
        Assert.assertEquals(javaInstanceStarter.useTls, Boolean.TRUE);
    }
    ...
    @Test
    public void invalidConfigFileShouldBeIgnored() throws IOException {
        JavaInstanceStarter javaInstanceStarter = new JavaInstanceStarter();
        String[] args = new CommandLineArgsBuilder()
                .withJCommanderConfigs(requiredJCommanderConfigs())
                .withJCommanderConfigs("--config_file", "nonexist.conf")
                .build();
        javaInstanceStarter.setConfigs(args);
        assertRequiredFieldsAreSet(javaInstanceStarter);
        assertOptionalFieldHasDefaultValue(javaInstanceStarter);
    ...
    @Test
    public void provideRequiredFieldsOnlyShouldSetValueProperlyAndOptionalFieldsAreSetToDefaultWithEmptyFile() throws IOException {
        JavaInstanceStarter javaInstanceStarter = new JavaInstanceStarter();
        String[] args = new CommandLineArgsBuilder()
                .withJCommanderConfigs(requiredJCommanderConfigs())
                .useFile()
    ...
    @Test
    public void provideRequiredFieldsFromFileShouldSuccess() throws IOException {
        JavaInstanceStarter javaInstanceStarter = new JavaInstanceStarter();
        String[] args = new CommandLineArgsBuilder()
    ...
    @Test
    public void provideFieldsFromFileAndCommandLineShouldSuccess() throws IOException {
        JavaInstanceStarter javaInstanceStarter = new JavaInstanceStarter();
        String[] args = new CommandLineArgsBuilder()
                .withJCommanderConfigs("--instance_id", INTEGER_INVALID_VALUE)
                .withFileConfigs(requiredFileConfigs())
                .exceptFileConfigWithKey("instanceId")
                .build();
        javaInstanceStarter.setConfigs(args);
        assertRequiredFieldsAreSet(javaInstanceStarter);
        assertOptionalFieldHasDefaultValue(javaInstanceStarter);
    ...
    @Test
    public void provideAllConfigsUsingCommandLineShouldSucess() throws IOException {
        JavaInstanceStarter javaInstanceStarter = new JavaInstanceStarter();
    ...
    @Test
    public void provideAllConfigsUsingFileShouldSucess() throws IOException {
        JavaInstanceStarter javaInstanceStarter = new JavaInstanceStarter();
        String[] args = new CommandLineArgsBuilder()
                .withFileConfigs(requiredFileConfigs())
    ...
    @Test
    public void commandLineArgsHasHigherPriorityThanFile() throws IOException {
        JavaInstanceStarter javaInstanceStarter = new JavaInstanceStarter();
        String[] args = new CommandLineArgsBuilder()
                .withJCommanderConfigs(requiredJCommanderConfigs())
                .withJCommanderConfigs(optionalJCommanderConfigs())
                .arityZeroBooleanInJCommander()
                .withFileConfigs(setRandomValueForConfigs(requiredFileConfigs()))
                .withFileConfigs(setRandomValueForConfigs(optionalFileConfigs()))
                .build();
        javaInstanceStarter.setConfigs(args);
        assertRequiredFieldsAreSet(javaInstanceStarter);
        assertOptionalFieldsAreSet(javaInstanceStarter);
    ...
    @Test
    public void emptyConfigsUsingFileShouldThrowIllegalArugmentException() throws IOException {
        JavaInstanceStarter javaInstanceStarter = new JavaInstanceStarter();
        String[] args = new CommandLineArgsBuilder()
                .useFile()
                .build();
        Assert.assertThrows(IllegalArgumentException.class, () -> javaInstanceStarter.setConfigs(args));
    }
    ...
    private void assertOptionalFieldsAreSet(JavaInstanceStarter javaInstanceStarter) {
        ...
        // Optional Boolean
        Assert.assertEquals(javaInstanceStarter.useTls, Boolean.TRUE);
        Assert.assertEquals(javaInstanceStarter.tlsAllowInsecureConnection, Boolean.FALSE);
        Assert.assertEquals(javaInstanceStarter.tlsHostNameVerificationEnabled, Boolean.TRUE);
        Assert.assertEquals(javaInstanceStarter.exposePulsarAdminClientEnabled, Boolean.TRUE);
    }

    private void assertOptionalFieldHasDefaultValue(JavaInstanceStarter javaInstanceStarter) {
        // Optional String
        Assert.assertEquals(javaInstanceStarter.jarFile, "");
        Assert.assertEquals(javaInstanceStarter.transformFunctionJarFile, "");
        Assert.assertEquals(javaInstanceStarter.transformFunctionId, "");
        Assert.assertEquals(javaInstanceStarter.clientAuthenticationPlugin, "");
        Assert.assertEquals(javaInstanceStarter.clientAuthenticationParameters, "");
        Assert.assertEquals(javaInstanceStarter.tlsTrustCertFilePath, "");
        Assert.assertEquals(javaInstanceStarter.stateStorageImplClass, "");
        Assert.assertEquals(javaInstanceStarter.stateStorageServiceUrl, "");
        Assert.assertEquals(javaInstanceStarter.secretsProviderClassName, "");
        Assert.assertEquals(javaInstanceStarter.secretsProviderConfig, "");
        ...
        // Optional Boolean
        Assert.assertEquals(javaInstanceStarter.useTls, Boolean.FALSE);
        Assert.assertEquals(javaInstanceStarter.tlsAllowInsecureConnection, Boolean.TRUE);
        Assert.assertEquals(javaInstanceStarter.tlsHostNameVerificationEnabled, Boolean.FALSE);
