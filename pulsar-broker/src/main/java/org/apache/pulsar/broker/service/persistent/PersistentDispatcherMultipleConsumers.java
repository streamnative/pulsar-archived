    ...
    public synchronized void removeConsumer(Consumer consumer) throws BrokerServiceException {
        ...
        if (consumerSet.removeAll(consumer) == 1) {
            ...
            if (consumerList.isEmpty()) {
                ...
                consumer.getPendingAcks().forEach((pair1, pair2) -> {
                    messagesToRedeliver.add(pair1.first, pair1.second);
                    redeliveryTracker.addIfAbsent(PositionImpl.get(pair1.first, pair1.second));
                });
        ...
        while (entriesToDispatch > 0 && totalAvailablePermits > 0 && isAtleastOneConsumerAvailable()) {
            ...
            if (messagesForC > 0) {
                ...
                try {
                    ...
                    log.error("Failed to filter entries for consumer {}", c.consumerName(), e);
                    cursor.rewind();
                    break;
                }
